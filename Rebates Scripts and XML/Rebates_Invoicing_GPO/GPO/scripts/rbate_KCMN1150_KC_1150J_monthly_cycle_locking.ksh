...
if [[ $REGION = "prod" ]];   then                                            
  if [[ $QA_REGION = "true" ]];   then                                       
    ...
    EMAIL_LIST="GDXDSCOps@caremark.com,GDXGPOOps@caremark.com"
  fi                                                                         
else                                                                         
...
SET RBATE_CYCLE_STATUS = 'L'
   ,UPD_DATE = SYSDATE
   ,CYCLE_LOCK_DT =  SYSDATE
WHERE RBATE_CYCLE_TYPE_ID = '1'
  AND RBATE_CYCLE_GID = (SELECT MAX(RBATE_CYCLE_GID)
...
SET RBATE_CYCLE_STATUS = 'A'
   ,UPD_DATE = SYSDATE 
WHERE RBATE_CYCLE_TYPE_ID = '1'
  AND RBATE_CYCLE_GID = (SELECT MIN(RBATE_CYCLE_GID)
                           FROM DMA_RBATE2.T_RBATE_CYCLE
                          WHERE RBATE_CYCLE_TYPE_ID = 1
                            AND   RBATE_CYCLE_STATUS IN ('I'))
;
