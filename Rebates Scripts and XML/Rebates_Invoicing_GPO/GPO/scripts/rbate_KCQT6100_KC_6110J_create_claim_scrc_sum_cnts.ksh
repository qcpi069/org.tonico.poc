...
export JOB="KC_6110J"
export FILE_BASE="rbate_"$SCHEDULE"_"$JOB"_create_claim_scrc_sum_cnts"
...
update dma_rbate2.t_claim_sum_cnt a 
   set (scrc_cnt, scrc_excpt_cnt) = 
       (select /*+ full(scrc) parallel(scrc,16) */ 
         count(*)
        ,SUM(CASE
             when excpt_id not in (10,11,12) 
...
if [[ $ORA_RETCODE = 0  ]]; then
   ...
   print " " 									>> $OUTPUT_PATH/$LOG_FILE
   print "Failure in update of scrc sum counts "       				>> $OUTPUT_PATH/$LOG_FILE
