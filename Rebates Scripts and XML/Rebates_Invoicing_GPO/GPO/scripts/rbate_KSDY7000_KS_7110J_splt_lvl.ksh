#!/bin/ksh
#-------------------------------------------------------------------------#
# Script        : rbate_KSDY7000_KS_7110J_splt_lvl.ksh   
# Title         : .
#
# Description   : Extract Split Level Data 
#                 
#                 
# Maestro Job   : KSDY7000 KS7110J
#
# Parameters    : none 
#                
#
...
if [[ $REGION = "prod" ]];   then
   ...
   if [[ $QA_REGION = "true" ]]; then
     ...
     export MVS_DSN="test.d"
       ...
       SCHEDULE="KSDY7000"
       JOB="KS_7110J"
       FILE_BASE="rbate_"$SCHEDULE"_"$JOB"_split_level_extrct"
       FILE_NAME=$JOB".PCS.P.KSZ7001J.KSCC017"
       ...
       TARGET_FILE=$MVS_DSN".ksz7001j.kscc017"
...
print "SPLIT LEVEL EXTRACT " >> $OUTPUT_PATH/$LOG_FILE
print ' ' >> $OUTPUT_PATH/$LOG_FILE

#-------------------------------------------------------------------------#
# SPLITS LEVEL EXTRACT
# Set up the Pipe file, then build and EXEC the new SQL.               
#                                                                         
#-------------------------------------------------------------------------#
print `date` 'Beginning select of SPLIT LEVEL Extract ' >> $OUTPUT_PATH/$LOG_FILE
...
SELECT 
   NVL(LPAD(splt_lvl_id,11,'0'),'00000000000')
  ,NVL(LPAD(pcs_system_id,11,'0'), '00000000000')
  ,NVL(LPAD(rebate_split_id,11,'0'),'00000000000')
  ,NVL(rpad(clm_typ_qlfr_vl,2,' '),'  ')
  ,NVL(rpad(rtrim(update_userid),10,' '),'          ')
  ,rpad(to_char(insrt_date,'yyyy-mm-dd-hh24.mm.ss'),26,'.000000')
FROM rbate_reg.split_level 
order by pcs_system_id, rebate_split_id,clm_typ_qlfr_vl;
