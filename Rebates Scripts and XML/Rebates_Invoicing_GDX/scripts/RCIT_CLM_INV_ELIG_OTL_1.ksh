...
#   usage: update_breakpoint_table  
{
	 ...
	 sql="update VACTUATE.RCIT_CLM_INV_ELIG_OTL_1 
...
function update_pre_count
{
         ...
         sql="UPDATE VACTUATE.RCIT_CLM_INV_ELIG_OTL_1 A
...
#   usage: create_update_query  
{
    sql="MERGE INTO VRAP.RCIT_CLM_INV_ELIG a
	 USING (SELECT AP_CO_CD
		     ,CLM_GID
		     ,BTCH_GID,
		     BUS_SEG_CD
	       FROM VRAP.RCIT_BASE_CLM_INV
	       WHERE AP_CO_CD IS NOT NULL
	       AND   model_typ_cd = '$model'
	       AND   INV_ELIG_DT = '$dt'
	       AND   btch_gid = $gid) b
	ON A.CLM_GID = B.CLM_GID
	AND A.INV_ELIG_DT >= '2009-04-30'
	AND model_typ_cd ='$model'
	AND INV_ELIG_DT = '$dt'
	AND a.btch_gid = b.btch_gid
	WHEN MATCHED AND a.DSPN_DT < '2013-01-01'
	THEN
	  UPDATE SET A.AP_CO_CD = B.AP_CO_CD,
		     A.CLT_ID = A.RBAT_ID,
...
#************************************************
   ...
   sql="select * from VACTUATE.RCIT_CLM_INV_ELIG_OTL_1 where STATUS = 'NOT STARTED' or STATUS = 'ERROR'"
