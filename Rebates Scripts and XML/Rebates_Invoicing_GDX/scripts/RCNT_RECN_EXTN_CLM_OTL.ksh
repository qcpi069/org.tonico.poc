...
#   usage: update_breakpoint_table
{
         ...
         sql="update VACTUATE.RCNT_RECN_EXTN_CLM_OTL
...
function update_pre_count
{
         ...
         sql="UPDATE VACTUATE.RCNT_RECN_EXTN_CLM_OTL A
		SET (A.PRE_UPDATE_COUNT) = (select count(*) 
			 from VRAP.RCNT_RECN_EXTN_CLM B
			 where A.pd = B.period_id and B.RECN_GID = A.RECN_GID
		     group by b.period_id, B.RECN_GID) 
...
function update_post_count
{
		...
		SET (A.POST_UPDATE_COUNT) = (select count(*) 
			 from VRAP.RCNT_RECN_EXTN_CLM B
			 where B.CLT_ID = B.RBAT_ID and B.CLT_NM = B.CLNT_NM 
			 and A.pd = B.period_id and B.RECN_GID = A.RECN_GID
		     group by b.period_id,B.RECN_GID)"
...
{
    sql="MERGE INTO VRAP.RCNT_RECN_EXTN_CLM a
      USING (SELECT BUS_SEG_CD, RBAT_ID
             FROM CLIENT_REG.CRT_CLNT) b
      ON A.RBAT_ID = B.RBAT_ID 
      AND A.PERIOD_ID = $pd
      AND A.RECN_GID = $recnid
      WHEN MATCHED 
      THEN
...
#************************************************
   ...
   sql="select * from VACTUATE.RCNT_RECN_EXTN_CLM_OTL where STATUS = 'NOT STARTED' or STATUS = 'ERROR'"
   ...
   if [[ $RETCODE = 0 ]]; then
      ...
      # uncomment the below line in order to populate pre count each time before execution for records where 
      # pre count is null i.e. not calculated yet.
      #update_pre_count
      
      #****************************************************************
      # Continue below logic to update records
      #****************************************************************
      while read table pd recnid PRE_UPDATE_COUNT POST_UPDATE_COUNT START_TIME END_TIME STATUS; do
         print "\nStart updation or records for the below mentioned criteria " >> $LOG_FILE
         print "TABLE NAME = " ${table} >> $LOG_FILE
         print "PROCESS ID  = " ${pd} >> $LOG_FILE
         print "RECON ID  = " ${recnid} >> $LOG_FILE
         ...
         export precount='PRE_UPDATE_COUNT'
