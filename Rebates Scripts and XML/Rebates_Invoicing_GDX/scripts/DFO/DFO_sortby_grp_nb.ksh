...
echo "Sorting claims records by GROUP NUMBER/FILL DATE.."
...
#===================================================================
  ...
  if [[ ! -f "$IN_DATA_FILE" ]] then
     ...
     echo "Script: 'DFO_sortby_grp_nb.ksh'"                 \
...
#===================================================================
  ...
  sort -t"^" -T /GDX/prod/tmp -k1.180,1.194 -k1.34,1.41 $IN_DATA_FILE -o $OUT_DATA_FILE
  
  if [[ $? != 0 ]] then
  
     MAIL_SUBJECT="DFO SCRIPT EXECUTION ERROR"
     echo "DFO script 'DFO_sortby_grp_nb.ksh'"  \
          "\nProcessing for $CLIENT_NAME"       \
  ...
  echo "Claims records were sorted by GROUP NUMBER/FILL DATE.."
