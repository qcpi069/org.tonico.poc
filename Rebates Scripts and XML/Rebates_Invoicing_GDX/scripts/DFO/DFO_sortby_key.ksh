...
echo "Sorting claims records by NABP/FII-DT/REFILL-CD/RX-NB.."
...
#===================================================================
  ...
  if [[ ! -f "$IN_DATA_FILE" ]] then
     ...
     echo "Script: 'DFO_sortby_key.ksh'"                    \
...
#===================================================================
  ...
  sort -t"^" -T /GDX/prod/tmp -k1.2,1.8 -k1.36,1.43 -k1.85,1.86 -k1.344,1.352 \
            $IN_DATA_FILE -o $OUT_DATA_FILE
  
  if [[ $? != 0 ]] then
  
     MAIL_SUBJECT="DFO SCRIPT EXECUTION ERROR"
     echo "DFO script 'DFO_sortby_key.ksh'"  \
          "\nProcessing for $CLIENT_NAME"    \
  ...
  echo "Claims records were sorted.."
