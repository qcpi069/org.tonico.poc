...
#        should be just the one warning message here, which is expected & ok
         ...
         echo "File was succussfully transferred to dwhtest1, YEAH!.." >> $LOG_FILE
	 cat $FTP_ERROR_LOG
      else 	 
...
#     we didn't find the warning msg, other issues
      ...
      print "Script: $SCRIPT_NAME"                          \
           "\nProcessing for $CLIENT_NAME Failed:"          \
           "\nftp of .ok for load to dwhtest1 had errors"    \
           "\n(no warning message) "                        \
           "\nLook for Log file $LOG_FILE" > $MAILFILE
...
#        should be just the one warning message here, which is expected & ok
         ...
         echo ".ok file was succussfully transferred to dwhtest1, YEAH!.." >> $LOG_FILE
	 cat $FTP_ERROR_LOG
         return 222
      else 	 
#        There's more than just 1 line in the error_log... we expect just 1 warn msg
         ...
         print "Script: $SCRIPT_NAME"                            \
              "\nProcessing for $CLIENT_NAME Failed:"            \
              "\nftp of .ok for load file to dwhtest1 had errors" \
              "\nLook for Log file $LOG_FILE" > $MAILFILE
