...
#   usage: update_breakpoint_table  
{
	 ...
	 sql="update VACTUATE.RCIT_CLM_INV_ELIG_OTL_2 
...
function update_pre_count
{
         ...
         sql="UPDATE VACTUATE.RCIT_CLM_INV_ELIG_OTL_2 A
		SET (A.PRE_UPDATE_COUNT) = (select count(*) 
			 ...
			 B.INV_ELIG_DT = A.INV_ELIG_DT AND
	                 B.NDC_PART_ID = A.BATCH_GID
		   group by b.model_typ_cd,B.INV_ELIG_DT,B.NDC_PART_ID) 
...
#************************************************
   ...
   sql="select * from VACTUATE.RCIT_CLM_INV_ELIG_OTL_2 where STATUS = 'NOT STARTED' or STATUS = 'ERROR'"
   ...
   if [[ $RETCODE = 0 ]]; then
      ...
      #****************************************************************  
      while read model dt ndcid TOTAL_COUNT PRE_UPDATE_COUNT POST_UPDATE_COUNT START_TIME END_TIME STATUS; do
         print "\nStart updation or records for the below mentioned criteria " >> $LOG_FILE
         print "MODEL = " ${model} >> $LOG_FILE  
         print "DATE  = " ${dt} >> $LOG_FILE 
         print "ID    = " ${ndcid} >> $LOG_FILE 
