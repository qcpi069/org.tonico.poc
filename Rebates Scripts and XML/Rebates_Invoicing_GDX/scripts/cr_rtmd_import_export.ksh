...
#   Date                 Description
# ---------  ----------  -------------------------------------------------#
# 09-28-16   qcpue98u    Change FTP to SFTP
#------------------------------------------------------------------------------
...
function find_basedir {
	...
	for dir in ../java/clientreg; do
#	for dir in ../java ../java/clientreg; do
...
#--------------------------------------------------------#
#	SFTP the POS rate files                          #
#--------------------------------------------------------#

exportDir="/GDX/prod/java/clientreg/data/export"

propfile=$(echo $FILE | awk -F "." '{print $2}')
platform=$(echo $propfile | awk -F "_"  '{print $1}')
filePattern=$(echo $propfile | awk 'BEGIN{OFS=FS="_"}{a=$1; for (i=2; i<=NF; i++) $(i-1)=$i; $NF=a}1')

case $filePattern in

        MFG_PMCY_EXCL_$platform)
                SCRIPT=CR_RTMD_IMPORT_EXPORT.MFG_PMCY_EXCL_$platform
        ;;
        MFG_RATE_$platform)
               SCRIPT=CR_RTMD_IMPORT_EXPORT.MFG_RATE_$platform
        ;;
        MFG_ST_EXCL_$platform)
               SCRIPT=CR_RTMD_IMPORT_EXPORT.MFG_ST_EXCL_$platform
        ;;

        POS_FEED_$platform)
              SCRIPT=CR_RTMD_IMPORT_EXPORT.POS_CLT_$platform
	      filePattern=POS_CLT_$platfrom
        ;;
esac

FileName=`ls -p $exportDir/*$filePattern*`
sftpFile=`basename $FileName`


	$REBATES_HOME/scripts/Common_SFTP_Process.ksh -p $SCRIPT -s $sftpFile -t $sftpFile
