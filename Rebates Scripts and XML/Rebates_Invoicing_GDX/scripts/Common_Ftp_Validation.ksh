...
# look for optional parm for alternate location of data source
if [[ $1 != "" ]] ; then
   FTP_VAL_DATA_PATH=$1
else
   FTP_VAL_DATA_PATH=$INPUT_PATH
fi
...
if [[ ${SUBSYSTEM:-none} != "none" ]] ; then
   FTP_DATA_FILE=$FTP_VAL_DATA_PATH/$SUBSYSTEM/${CFV_FTP_DATA_FILE_FULL##*/}
else
   FTP_DATA_FILE=$FTP_VAL_DATA_PATH/${CFV_FTP_DATA_FILE_FULL##*/}
fi
