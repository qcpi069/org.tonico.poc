...
ftp -n tsmnim1 <<-DELIM >$FTP_LOG 2>$FTP_ERROR_LOG
...
#     we didn't find the warning msg, other issues
      ...
      MAIL_SUBJECT="MDA ftp to tsmnim1 PROCESS"
...
#        should be just the one warning message here, which is expected & ok
         ...
         echo "File was successfully transferred to tsmnim1, YEAH!.." >> $LOG_FILE
	 cat $FTP_ERROR_LOG
      else 	 
...
#     we didn't find the warning msg, other issues
      ...
      print "Script: $SCRIPT_NAME"                             \
           "\nProcessing for $RUN_MODE claims intake Failed:"  \
           "\nftp of .ok file for load to tsmnim1 had errors"  \
           "\n(no warning message) "                           \
           "\nLook for Log file $LOG_FILE" > $MAILFILE

      MAIL_SUBJECT="MDA ftp to tsmnim1 (.ok) PROCESS"
      $SCRIPT_DIR/mailto_IS_group.ksh
...
#        should be just the one warning message here, which is expected & ok
         ...
         echo ".ok file was successfully transferred to tsmnim1, YEAH!.." >> $LOG_FILE
	 cat $FTP_ERROR_LOG
         return 222
      else 	 
#        There's more than just 1 line in the error_log... we expect just 1 warn msg
