...
#===================================================================
  ...
  export LOAD_TRIGGER_FILE="$TEMP_DATA_DIR/trigger.dat"

##  dwgdx001 is shorter extract record (no COPAY)
##  $SCRIPT_DIR/MDA_dwgdx001.ksh >>$LOG_FILE 2>&1

##  dwgdx002 has COPAY
  $SCRIPT_DIR/MDA_dwgdx002.ksh >>$LOG_FILE 2>&1
...
# 222 is a good return from FTP kornshell... test for dba load script status
  if (($FTP_STATUS==222)); then
      ...
      echo    "Load Processing for $RUN_MODE claims intake is starting:"   \
              "\nload file move to DBA directory was good."                \
              "\nfollow progress in $LOG_FILE" > $MAILFILE

##              "\nftp of load file to dwhtest1 was good."                   \
 
      MAIL_SUBJECT="MDA db load PROCESS"
      $SCRIPT_DIR/mailto_IS_group.ksh

      $SCRIPT_DIR/MDA_load_tclaims_integration.ksh >>$LOG_FILE 
...
###
###         not needed for new integration environment
###
