...
#  db2 "connect to udbmdap user " $CHESTER_USER " using " $CHESTER_PASSWORD" >/dev/null 2>/dev/null
 ...
 SQL_TEXT="select 'rtn_code', ',' ,ret_code from dba.loads where data_file = '$DATA_LOAD_DIR/$DATA_LOAD_FILE'"
  echo "SQL TEXT " $SQL_TEXT

  db2 "select 'rtn_code', ',' ,ret_code from dba.loads where data_file = '$DATA_LOAD_DIR/$DATA_LOAD_FILE'"> $DB_2_LOG 2>>$LOG_FILE
...
# we'll delete the db2 log file to break the loop
  ...
  echo "in test Load_Return_Code: " $Load_Return_Code
  while [[ -s $DB_2_LOG ]]
  do
...
#       row into DBA.LOADS.  We'll use a counter to limit the time we wait.
        ...
        if (($LOOP_CTR>03)); then
           echo "Load Processing for $RUN_MODE claims intake had problems:"     \
                ...
                "\n$DBA_DATA_LOAD_DIR/$DATA_LOAD_FILE "                         \
                "\nLoop counter exceeded limit of 03 min wait"                  \
                "\nreference $LOG_FILE" > $MAILFILE
  ...
  echo "in test at end Load_Return_Code: " $Load_Return_Code
  echo "\n***********************************************"   >> $LOG_FILE
