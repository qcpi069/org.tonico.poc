#!/bin/ksh
#-------------------------------------------------------------------------#
# Script        : GDX_GDDY0010_formulary_expansion_check.ksh   
# Title         : Calls the java job that checks to see if the users have
#                 scheduled formulary expansion.
#
#                 
# This script is executed from the following Maestro jobs:
# Maestro Job   : GDDY0010 / GD_0060J
#
# Parameters    : None.
#
...
SCHEDULE="GDDY0010"
JOB="GD_0060J"
DFLT_FILE_BASE="GDX_"$SCHEDULE"_formulary_expansion_check"
...
FILE_BASE="GDX_"$SCHEDULE"_formulary_expansion_check"
...
if [[ $RETCODE = 0 ]]; then
   ...
   print "$JAVACMD" "-Dlog4j.configuration=log4j.properties" "-DlogFile=${LOG_FILE}" "-DREGION=${REGION}" "-DQA_REGION=${QA_REGION}" com.caremark.gdx.FormularyExpansionCheck.FormularyExpansionCheck >> $LOG_FILE

   "$JAVACMD" "-Dlog4j.configuration=log4j.properties" "-DlogFile=${LOG_FILE}" "-DREGION=${REGION}" "-DQA_REGION=${QA_REGION}" com.caremark.gdx.FormularyExpansionCheck.FormularyExpansionCheck >> $LOG_FILE
   export RETCODE=$?
