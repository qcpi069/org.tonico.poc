#!/bin/ksh
#-------------------------------------------------------------------------#
# Script        : MDA_Allocation_MKTSHR_qtrly_cntrc_sql040.ksh   
# Title         
#
# Description   : This script is for reallocating contract in the 
#                   market share quarterly billed table, TMKTSHR_BILLED_SUM.
#
   ...
   ,VRAP.TRPT_REQMT E
    WHERE PART1.CNTRCT_ID                       =   C.CNTRCT_ID
      ...
      AND E.DRUG_CLS_VNDR       =   D.MDA_VNDR_DCL_ID
       AND E.RPT_ID                     IN  (SELECT C.RPT_ID 
                               FROM VRAP.TRPT_REQMT C
...
--    DON'T OVERLAY THE ALREADY ADDED MONTHLY BILLED VALUES
      ...
      AND ( ('M' || PART1.NEW_PERIOD_MONTH || PART1.NEW_PERIOD_YEAR) ||
              ...
              (SELECT DISTINCT ( RTRIM(CHAR(MS1.PERIOD_ID))  ||
                                 RTRIM(CHAR(MS1.CNTRCT_ID))  ||
                                 RTRIM(CHAR(MS1.RPT_ID))        )
                       FROM VRAP.TMKTSHR_BILLED_SUM MS1)
;
...
if [[ $RETCODE != 0 ]]; then
    ...
    print "script MDA_Allocation_MKTSHR_qtrly_cntrc_sql040.ksh " >> $LOG_FILE
