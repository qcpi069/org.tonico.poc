#!/bin/ksh
#-------------------------------------------------------------------------#
# Script        : MDA_Allocation_MKTSHR_mnthly_bill_sql010.ksh   
# Title         
#
...
DELETE FROM $SCHEMA_OWNER.TMKTSHR_BILLED_SUM
 WHERE PERIOD_ID      = '$PERIOD_ID' 
   AND RPT_ID IN (SELECT RPT_ID 
                    FROM VRAP.TMKTSHR_CLAIM_SUM
-- Changed based on Integration testing
--                    FROM VRAP.TDISCNT_CLAIM_SUM
                    WHERE PERIOD_ID      = '$PERIOD_ID'
...
if [[ $RETCODE != 0 ]]; then
    ...
    print "script MDA_Allocation_MKTSHR_mnthly_bill_sql010.ksh " >> $LOG_FILE
