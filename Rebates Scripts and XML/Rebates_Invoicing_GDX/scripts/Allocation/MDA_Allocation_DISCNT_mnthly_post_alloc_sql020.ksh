#!/bin/ksh
#-------------------------------------------------------------------------#
# Script        : MDA_Allocation_DISCNT_mnthly_post_alloc_sql020.ksh   
# Title         
#
...
FROM 
 ...
 ,A.QUAL_UNITS             AS QUAL_UNITS
  ...
  where a.period_id = '$PERIOD_ID'
    and a.discnt_run_mode = 'MPRD')  AS PART1
 WHERE NOT EXISTS 
      (SELECT 1
      FROM $SCHEMA_OWNER.TDISCNT_BILLED_SUM$MODEL  B   
...
if [[ $RETCODE != 0 ]]; then
    ...
    print "script MDA_Allocation_DISCNT_mnthly_post_alloc_sql020.ksh " >> $LOG_FILE
