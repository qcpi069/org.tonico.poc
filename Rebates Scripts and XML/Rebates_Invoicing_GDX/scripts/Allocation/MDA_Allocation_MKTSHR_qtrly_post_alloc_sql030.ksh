#!/bin/ksh
#-------------------------------------------------------------------------#
# Script        : MDA_Allocation_MKTSHR_qtrly_post_alloc_sql030.ksh   
# Title         
#
...
FROM 
 (SELECT A.CNTRCT_ID
   ...
   FROM $SCHEMA_OWNER.TMKTSHR_BILLED_SUM A
       ,vrap.tdiscnt_period r
  where a.period_id = r.period_id
    and r.period_parent_id = '$PERIOD_ID'

-- for report specific if needed
--      and a.rpt_id in ($REPORT_ID)
-- examples
--      and r.period_parent_id = 'Q0404'
--      and a.rpt_id in (2545, 2645)
-- this parent_id value expands out to three months
-- like " where a.period_id in ('M1004','M1104','M1204') "
--
--  Not TAP (monthly billed) 
    and a.cntrct_id <> 78

-- or not any other 'MPRD'
        and a.rpt_id not in (
                select distinct s.rpt_id
                  from vrap.tmktshr_claim_sum s
                      ,vrap.tdiscnt_period r
                 where s.period_id = r.period_id
                   and r.period_parent_id = '$PERIOD_ID'
                   and discnt_run_mode = 'MPRD' )

    AND A.RPT_ID NOT IN
    (SELECT X.TMP_RPT_ID FROM VACTUATE.TMP_RPT_DLVRY_SYS  X
      WHERE X.TMP_PERIOD_ID  in ('$PERIOD_ID')
...
if [[ $RETCODE != 0 ]]; then
    ...
    print "script MDA_Allocation_MKTSHR_qtrly_post_alloc_sql030.ksh " >> $LOG_FILE
