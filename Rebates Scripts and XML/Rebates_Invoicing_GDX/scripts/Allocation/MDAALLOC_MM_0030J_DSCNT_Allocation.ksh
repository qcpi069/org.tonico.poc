#!/bin/ksh
#-------------------------------------------------------------------------#
# Script        : MDAALLOC_MM_0030J_DSCNT_Allocation.ksh 
...
SCHEDULEA="MDAALLOC"
JOBA="MM_0030J"
FILE_BASEA=$SCHEDULEA"_"$JOBA"_""DSCNT_Allocation_"$PERIOD_IDA"_"$CNTRCT_IDA"_"$RPT_IDA
...
DESC_TEXTA="DSCT30J $DISCNT_RUN_MODE_CDA C$CNTRCT_IDA R$RPT_IDA"
...
if [[ $SQLCODE != 0 ]]; then
      print "Script " $SCRIPTNAMEA "failed in Update TALLOCATN_SCHEDULEA to REQ_STAT = 'R'." >> $LOG_FILEA
...
if [[ $DISCNT_RUN_MODE_CDA = 'ACCL' ]]; then
   print " " >> $LOG_FILEA
   print " " >> $LOG_FILEA
   print "!!!!!! Running MDAALLOC_MM_0030JA_DSCNT_Allocation.ksh" >> $LOG_FILEA
   print " " >> $LOG_FILEA
   print " " >> $LOG_FILEA
   . $SCRIPT_PATH/MDAALLOC_MM_0030JA_DSCNT_Allocation.ksh $MODEL_TYP_CD
   RETCODE=$?
fi
...
if [[ $DISCNT_RUN_MODE_CDA = 'MPRD' ]]; then
   print " " >> $LOG_FILEA
   print " " >> $LOG_FILEA
   print "!!!!!! Running MDAALLOC_MM_0030JB_DSCNT_Allocation.ksh" >> $LOG_FILEA
   print " " >> $LOG_FILEA
   print " " >> $LOG_FILEA
   . $SCRIPT_PATH/MDAALLOC_MM_0030JB_DSCNT_Allocation.ksh $MODEL_TYP_CD
   RETCODE=$?
fi
...
if [[ $DISCNT_RUN_MODE_CDA = 'PROD' ]]; then
   print " " >> $LOG_FILEA
   print " " >> $LOG_FILEA
   print "!!!!!! Running MDAALLOC_MM_0030JC_DSCNT_Allocation.ksh" >> $LOG_FILEA
   print " " >> $LOG_FILEA
   print " " >> $LOG_FILEA
   . $SCRIPT_PATH/MDAALLOC_MM_0030JC_DSCNT_Allocation.ksh $MODEL_TYP_CD
   RETCODE=$?
fi
  # check SQLCODE - if bad set RETCODE = to SQLCODE
     
print "!!!!!! Done MDAALLOC_MM_0030J****_DSCNT_Allocation.ksh" >> $LOG_FILEA
print "Prior to Post Alloc SQLCODE is :" $SQLCODE >> $LOG_FILEA
   
if [[ $RETCODE = 0 ]]; then
  # POST ALLOCATION
  print "POST ALLOCATION" >> $LOG_FILEA
  . $SCRIPT_PATH/MDAALLOC_MM_0041J_DSCNT_Post_Allocation.ksh $MODEL_TYP_CD
  RETCODE=$?
else
  # Update to Error
...
if [[ $RETCODE = 0 ]]; then
   ...
   DESC_TEXTA="DSCT30J COMP $DISCNT_RUN_MODE_CDA C$CNTRCT_IDA R$RPT_IDA"
...
99EOFSQLTEXT99
   ...
   if [[ $RETCODE = '9999' ]]; then
    DESC_TEXTA="DSCT30J OOB $DISCNT_RUN_MODE_CDA C$CNTRCT_IDA R$RPT_IDA"
   else
        DESC_TEXTA="E30JDSCT $DISCNT_RUN_MODE_CDA C$CNTRCT_IDA R$RPT_IDA"
   fi     
   print "=========================================================" >> $LOG_FILEA
