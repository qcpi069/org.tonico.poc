#!/bin/ksh
#-------------------------------------------------------------------------#
# Script        : MDA_Allocation_DISCNT_mnthly_post_alloc_sql030.ksh   
# Title         
#
# Description   : This script is post allocation validation sql for the  
#                   monthly billed discount table, TDISCNT_BILLED_SUM$MODEL.
#
...
print "============================================================================= "  >> $LOG_FILE
print "Starting build of SQL for MDA_Allocation_DISCNT_mnthly_post_alloc_sql030.ksh"  >> $LOG_FILE
print `date` >> $LOG_FILE
...
FROM 
 ...
 ,A.QUAL_UNITS             AS QUAL_UNITS
   FROM $SCHEMA_OWNER.TDISCNT_BILLED_SUM$MODEL A  
  where a.period_id in ('$PERIOD_ID')
    and cntrct_id = 78
    and a.rpt_id in (
 select distinct s.rpt_id
   from vrap.TDISCNT_CLAIM_SUM$MODEL s
  where period_id = '$PERIOD_ID' 
    and discnt_run_mode = 'MPRD' ))  AS PART1
 WHERE NOT EXISTS 
      (SELECT 1
...
if [[ $RETCODE != 0 ]]; then
    ...
    print "script MDA_Allocation_DISCNT_mnthly_post_alloc_sql030.ksh " >> $LOG_FILE
    print "Return Code is : " $RETCODE >> $LOG_FILE
    print `date` >> $LOG_FILE
else    
    print " " >> $LOG_FILE
    print "....Completed building of SQL for MDA_Allocation_DISCNT_mnthly_post_alloc_sql030.ksh ...."   >> $LOG_FILE
