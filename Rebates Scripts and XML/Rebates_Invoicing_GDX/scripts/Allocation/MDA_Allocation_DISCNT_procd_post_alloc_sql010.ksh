#!/bin/ksh
#-------------------------------------------------------------------------#
# Script        : MDA_Allocation_DISCNT_procd_post_alloc_sql010.ksh   
# Title         
#
...
--        max ( prfmc_discnt_fctr ) as maxperf, min (prfmc_discnt_fctr ) as minperf, 6004155
        max (contract_price) as maxcprice, min (contract_price) as mincprice
         from $SCHEMA_OWNER.TDISCNT_PROCD_SUM$MODEL q
...
if [[ $RETCODE != 0 ]]; then
    ...
    print "script MDA_Allocation_DISCNT_procd_post_alloc_sql010.ksh " >> $LOG_FILE
