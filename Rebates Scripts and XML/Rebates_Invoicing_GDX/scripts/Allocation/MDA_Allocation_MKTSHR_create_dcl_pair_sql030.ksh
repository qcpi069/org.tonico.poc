#!/bin/ksh
#-------------------------------------------------------------------------#
# Script        : MDA_Allocation_create_dcl_pair_sql030.ksh   
# Title         
#
# Description   : This script is the third step of the process that creates
#               drug class pairs.
#                 
...
insert into $SCHEMA_OWNER.TMKTSHR_DCL_PAIR
 (select 
   max(ISRT_DCL_PAIR_ID)
  ,max(ISRT_CMPTR_DCL_ID)
  ,max(ISRT_VNDR_DCL_ID)
  ,max(ISRT_DCL_PAIR_LBL)
 from VACTUATE.TMKTSHR_DCL_PAIR_TEMP
 group by GRP_BY_COL
 having ( 
  (max(ISRT_CMPTR_DCL_ID) > 0 and max(ISRT_VNDR_DCL_ID) > 0) and
  (max(ISRT_DCL_PAIR_ID)  > 0) ) );
...
if [[ $RETCODE != 0 ]]; then
    ...
    print "script MDA_Allocation_create_dcl_pair_sql030.ksh " >> $LOG_FILE
