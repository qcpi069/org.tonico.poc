#!/bin/ksh
#-------------------------------------------------------------------------#
# Script        : MDA_Allocation_DISCNT_mnthly_post_alloc_sql010.ksh   
# Title         
#
...
# ---------  ---------  --------  ----------------------------------------#
# 12015-05   is00084    6005148   Modified to include Medicare-D changes
...
from vrap.TDISCNT_CLAIM_SUM$MODEL a,
 (select distinct q.period_id, q.rpt_id as rpt, q.drug_ndc_id as ndc, sum (q.qual_rx_count) as totcount,
...
--  max ( prfmc_discnt_fctr ) as maxperf, min (prfmc_discnt_fctr ) as minperf,  6004155
 ...
 group by q.period_id, q.rpt_id, q.drug_ndc_id) b
where a.period_id = b.period_id
and a.rpt_id = b.rpt
and a.drug_ndc_id = b.ndc
and a.discnt_run_mode = 'MPRD'
...
if [[ $RETCODE != 0 ]]; then
    ...
    print "script MDA_Allocation_DISCNT_mnthly_post_alloc_sql010.ksh " >> $LOG_FILE
