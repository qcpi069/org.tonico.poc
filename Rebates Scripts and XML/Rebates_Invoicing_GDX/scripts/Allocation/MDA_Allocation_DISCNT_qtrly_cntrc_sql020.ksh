#!/bin/ksh
#-------------------------------------------------------------------------#
# Script        : MDA_Allocation_DISCNT_qtrly_cntrc_sql020.ksh   
# Title         
#
...
#           numbers back into the months that make up a quarter by contract id.
#
...
--prephizer     THEN  C.PRFMC_DISCNT_FCTR  * DECIMAL(PART1.QUAL_UNITS,18,5) *  C.QUAL_EXT_PRICE
        ...
        WHEN RPT_TYP_CD = 11
            THEN    DECIMAL(PART1.QUAL_UNITS,18,5) * ((C.QUAL_EXT_PRICE - C.CONTRACT_PRICE) 
--prephizer                  + (C.PRFMC_DISCNT_FCTR  * DECIMAL(PART1.QUAL_UNITS,18,5)) )
                             + ((CASE WHEN PART1.PRFMC_DISCNT_FCTR IS NULL THEN C.PRFMC_DISCNT_FCTR
                                      ELSE PART1.PRFMC_DISCNT_FCTR END) * DECIMAL(PART1.QUAL_UNITS,18,5)) )
               END )  AS PRFMC_DISCOUNT_AMT 
       ,SUM(    
             CASE  
...
--prephizer   FROM    VRAP.TDISCNT_EXT_CLAIM$MODEL B
  ...
  WHERE    B.PERIOD_ID                      =   '$PERIOD_ID'
    AND    B.RPT_ID                    IN   (SELECT C.RPT_ID 
...
if [[ $RETCODE != 0 ]]; then
    ...
    print "script MDA_Allocation_DISCNT_qtrly_cntrc_sql020.ksh   " >> $LOG_FILE
