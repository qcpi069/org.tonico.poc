#!/bin/ksh
#-------------------------------------------------------------------------#
# Script        : MDA_Allocation_MKTSHR_qtrly_cntrc_sql050.ksh   
# Title         
#
...
--                                    AND 9999
     AND NOT EXISTS
    (SELECT 1 
           FROM VRAP.TDRUG_MDA_AGG_RULE
          WHERE VRAP.TDRUG_MDA_AGG_RULE.DRUG_NDC_ID   =  VRAP.TMKTSHR_BILLED_SUM.DRUG_NDC_ID
        AND VRAP.TDRUG_MDA_AGG_RULE.NHU_TYP_CD    =  VRAP.TMKTSHR_BILLED_SUM.NHU_TYP_CD ) ;
...
if [[ $RETCODE != 0 ]]; then
    ...
    print "script MDA_Allocation_MKTSHR_qtrly_cntrc_sql050.ksh " >> $LOG_FILE
