#!/bin/ksh
#-------------------------------------------------------------------------#
# Script        : MDA_Allocation_MKTSHR_pre_allocation_cntrct_lvl_sql010.ksh   
# Title         
#
# Description   : This script builds the first sql for the pre-allocation checking process
                   for a specific contract id.
#
...
SELECT             PART1.CNTRCT_ID      AS EXT_CNTRCT_ID
 ...
 FROM 
    (SELECT        A.CNTRCT_ID
        ...
        AND A.RPT_ID          IN    (SELECT C.RPT_ID 
                       FROM VRAP.TRPT_REQMT C
                      WHERE C.CNTRCT_ID = ($CONTRACT_ID))
...
if [[ $RETCODE != 0 ]]; then
    ...
    print "MDA_Allocation_MKTSHR_pre_allocation_cntrct_lvl_sql010.ksh " >> $LOG_FILE
