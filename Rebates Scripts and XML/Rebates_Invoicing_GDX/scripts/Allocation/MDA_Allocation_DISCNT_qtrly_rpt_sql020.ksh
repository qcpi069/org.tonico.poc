#!/bin/ksh
#-------------------------------------------------------------------------#
# Script        : MDA_Allocation_DISCNT_qtrly_rpt_sql020.ksh   
# Title         
#
...
#           numbers back into the months that make up a quarter by report id.
#
...
INSERT INTO $SCHEMA_OWNER.TDISCNT_BILLED_SUM$MODEL
 SELECT     PART1.CNTRCT_ID
           ...
           ,PART1.CLT_ID
           ,PART1.DLVRY_SYS_CD
           ,('M' || PART1.NEW_PERIOD_MONTH || PART1.NEW_PERIOD_YEAR)    AS PERIOD_ID
           ,C.QUAL_EXT_PRICE
           ,0                                               AS RX_COUNT
           ,PART1.QUAL_RX_COUNT 
...
(SELECT  B.CNTRCT_ID                                AS CNTRCT_ID
           ...
           , RTRIM( CASE  
                           WHEN  (MONTH(B.BILLG_END_DT)) <= 9  THEN '0' || CHAR(MONTH(B.BILLG_END_DT))
                            ELSE CHAR(MONTH(B.BILLG_END_DT))
                      END)                  AS NEW_PERIOD_MONTH
           ,SUBSTR(CHAR(YEAR(B.BILLG_END_DT)),3,2)      AS NEW_PERIOD_YEAR
...
---  UNCOMMENT THE FOLLOWING 2 LINES IF PROCESS TAKES TOO LONG TO RUN.  RUN QUERY IN GROUPS OF CONTRACT IDS.
--   AND B.CNTRCT_ID                 BETWEEN   0
--                                           AND 999
GROUP BY 
            B.CNTRCT_ID
           ,B.RPT_ID
...
--postphizer
        ...
        ,VRAP.TDISCNT_CLAIM_SUM$MODEL C 
   ...
   WHERE  PART1.CNTRCT_ID                           =   C.CNTRCT_ID
         ...
         AND ('Q' || 
              CASE 
        WHEN PART1.NEW_PERIOD_MONTH BETWEEN   '01' AND '03'
            THEN '01'
        WHEN PART1.NEW_PERIOD_MONTH BETWEEN   '04' AND '06'
            THEN '02'
        WHEN PART1.NEW_PERIOD_MONTH BETWEEN   '07' AND '09'
            THEN '03'
        WHEN PART1.NEW_PERIOD_MONTH BETWEEN   '10' AND '12'
            THEN '04'
                          END
       || PART1.NEW_PERIOD_YEAR)                 
                            =   C.PERIOD_ID
         AND PART1.DISCNT_RUN_MODE          =   C.DISCNT_RUN_MODE

--       DON'T OVERLAY THE ALREADY ADDED MONTHLY BILLED VALUES

         AND ( ('M' || PART1.NEW_PERIOD_MONTH || PART1.NEW_PERIOD_YEAR) ||
                 RTRIM(CHAR(C.CNTRCT_ID))  ||
                 RTRIM(CHAR(C.RPT_ID))       )     NOT IN 
                    (SELECT DISTINCT ( RTRIM(CHAR(DS1.PERIOD_ID))  ||
                                       RTRIM(CHAR(DS1.CNTRCT_ID))  ||
                                       RTRIM(CHAR(DS1.RPT_ID))        )
                             FROM $SCHEMA_OWNER.TDISCNT_BILLED_SUM$MODEL DS1)
...
if [[ $RETCODE != 0 ]]; then
    ...
    print "script MDA_Allocation_DISCNT_qtrly_rpt_sql020.ksh   " >> $LOG_FILE
