#!/bin/ksh
#-------------------------------------------------------------------------#
# Script        : MDA_Allocation_DISCNT_procd_cntrc_sql040.ksh   
# Title         
#
...
SELECT   DISTINCT DRUG_NDC_ID
             ...
             ,CURRENT TIMESTAMP      
             ,USER
 FROM $SCHEMA_OWNER.TDISCNT_PROCD_SUM$MODEL
...
AND NOT EXISTS
    (SELECT 1 
          ...
          WHERE $SCHEMA_OWNER.TDRUG_MDA_AGG_RULE.DRUG_NDC_ID   =  $SCHEMA_OWNER.TDISCNT_PROCD_SUM$MODEL.DRUG_NDC_ID
        AND $SCHEMA_OWNER.TDRUG_MDA_AGG_RULE.NHU_TYP_CD    =  $SCHEMA_OWNER.TDISCNT_PROCD_SUM$MODEL.NHU_TYP_CD ) 
...
if [[ $RETCODE != 0 ]]; then
    ...
    print "script MDA_Allocation_DISCNT_procd_cntrc_sql040.ksh " >> $LOG_FILE
