...
#-------------------------------------------------------------------------#
#
# Script        : GDX_CASH_wf_m_Create_PRCS_CNTL_Entries_CASH_Claims_Detail_Load_CopyVoid.ksh   
# Title         : 
#
# Description   : Searches Cash Recon for Copy/Void Recon with Claims that have not been loaded to 
#                 Cash Recon as yet. Inserts a row into the Process Controller and updates
#                 the Recon Headers to say they are in load process.   
#
# Maestro Job   : RCDY3000 RC_3220J
#
# Parameters    : None
#
...
SCHEDULE="RCDY3000"
JOB="RC_3220J"
FILE_BASE="GDX_CASH_wf_m_Create_PRCS_CNTL_Entries_CASH_Claims_Detail_Load_CopyVoid"
...
SQL_STMNT="Select count(*) from vrap.rcnt_recn_hdr hdr where hdr.recn_stat_cd = 15 "
...
if [[ $nr_row_count -gt 0 ]]; then
   SQL_STMNT="select count(*) from vrap.rcit_prcs_cntl pcnt where pcnt.prcs_cd = 16 and pcnt.prcs_stat_cd in (1,2,5,6,7) "
   print " "  >> $LOG_FILE
   print "We have $nr_row_count New Recons. Now look to see if there are processes running or waiting "  >> $LOG_FILE
   print "SQL_STMNT is - $SQL_STMNT"  >> $LOG_FILE
   db2 -x $SQL_STMNT > $ROW_CNT_FILE
   RETCODE=$?
   if [[ $RETCODE != 0 ]]; then
   ...
   if [[ $prcs_row_count -eq 0 ]]; then
    ...
    . $SCRIPTS_DIR/infa_pmcmd.ksh wf_m_Create_PRCS_CNTL_Entries_CASH_Claims_Detail_Load_CopyVoid 
      RETCODE=$?
   else
      print " "  >> $LOG_FILE
      print "There are Copy/Void Recons for which to Load claims into the Recon Claim table "  >> $LOG_FILE 
   ...
   print " "  >> $LOG_FILE
   print "There were no Copy/Void Recons for which to Load claims into the Recon Claim table"  >> $LOG_FILE 
   print " "   >> $LOG_FILE
   print "********************************************"   >> $LOG_FILE
   print "Completing the script $SCRIPTNAME ..." `date +"%D %r %Z"` >> $LOG_FILE
   print "********************************************"   >> $LOG_FILE   
