...
#-------------------------------------------------------------------------#
#
# Script        : GDX_Run_CASH_wf_m_Create_PRCS_CNTL_Entries_CASH_Send_PreAPC_Snapshot.ksh   
# Title         : 
#
# Description   : Searches Cash Recon for Pre APC Distribution Transactions that are ready
#                 for transmission.   
#
# Maestro Job   : GD_3240 
#
...
SCHEDULE="GDDY3000"
JOB="GD_3240"
FILE_BASE="GDX_Run_CASH_wf_m_Create_PRCS_CNTL_Entries_CASH_Send_PreAPC_Snapshot"
...
SQL_STMNT="Select count(*) from vrap.rcnt_recn_dstr_hdr hdr where hdr.dstr_stat_cd = 10 and hdr.dstr_crte_typ_cd = 1 "
...
if [[ $nr_row_count -gt 0 ]]; then
   SQL_STMNT="select count(*) from vrap.rcit_prcs_cntl pcnt where pcnt.prcs_cd = 18 and pcnt.prcs_stat_cd in (1,2,5,6,7) "
   print " "  >> $LOG_FILE
   print "We have $nr_row_count New Pre APC Distributions. Now look to see if there are processes running or waiting "  >> $LOG_FILE
   print "SQL_STMNT is - $SQL_STMNT"  >> $LOG_FILE
   db2 -x $SQL_STMNT > $ROW_CNT_FILE
   RETCODE=$?
   if [[ $RETCODE != 0 ]]; then
   ...
   if [[ $prcs_row_count -eq 0 ]]; then
    ...
    . $SCRIPTS_DIR/infa_pmcmd.ksh wf_m_Create_PRCS_CNTL_Entries_CASH_Send_PreAPC_Snapshot &
      RETCODE=$?
   else
      print " "  >> $LOG_FILE
      print "There are new PRE APC Distributions that are in Ready To Tranmitt status "  >> $LOG_FILE 
   ...
   print " "  >> $LOG_FILE
   print "There were no new PRE APC Distributions that are in Ready To Tranmitt status"  >> $LOG_FILE 
   print " "   >> $LOG_FILE
   print "********************************************"   >> $LOG_FILE
   print "Completing the script $SCRIPTNAME ..." `date +"%D %r %Z"` >> $LOG_FILE
   print "********************************************"   >> $LOG_FILE   
