...
# ---------  ----------  -------------------------------------------------
# 11-20-15    qcpi733    Added compress_File function and associated logic
#                        for ITPR012352 Coventry
...
#Check Archive Directory
                if [[ -z $ARCHIVE_DIR_TXT || $ARCHIVE_DIR_TXT == '-' ]]; then
                      ...
                      if [[ -d ${ARCHIVE_DIR_TXT} ]]; then
                           ...
                           if [[ COMPRESS_IND -eq 1 ]]; then
                           # It will just bulid the Archive command file since every records have flag set for Archive #
                           print "mv ${SOURCE_DIR_TXT}/$FINAL_TARGET $ARCHIVE_DIR_TXT/$FINAL_TARGET.`date +"%Y%j%H%M%S"`" >> $ARCH_OPTION_FILE
                           else 
...
print " "                                                                      >> $LOG_FILE
print "Compression indicator set to 1, use Gzip to compress file into Target name" >> $LOG_FILE
#cd $SCRIPTS_DIR

#`dirname $0`/Common_Gzip_Process.ksh -a c -d $SOURCE_DIR_TXT -s $FINAL_SOURCE -D $SOURCE_DIR_TXT -t $FINAL_TARGET
$SCRIPTS_DIR/Common_Gzip_Process.ksh -a c -d $SOURCE_DIR_TXT -s $FINAL_SOURCE -D $SOURCE_DIR_TXT -t $FINAL_TARGET


RC=$?

if [[ $RC != 0 ]]; then
    print "Error returned from Common_Gzip_Process, return code->$RC<"         >> $LOG_FILE
    print "Values used:"                                                       >> $LOG_FILE
    ...
    print " "                                                                  >> $LOG_FILE
    exit_script $RC "Error executing Gzip compression"
fi

print "done with gzip" >> $LOG_FILE

#At this point the $FINAL_SOURCE value (uncompressed) has to be replaced with the $FINAL_TARGET filename
#  so that when the SFTP command is built it uses the compressed filename in the put statement
FINAL_SOURCE=$FINAL_TARGET

print " "                                                                      >> $LOG_FILE
print "Compression execution successfully built $SOURCE_DIR_TXT/$FINAL_SOURCE" >> $LOG_FILE
print " "                                                                      >> $LOG_FILE
...
LOG=${PARENTNAME_TMP}"_SFTP".log
LOG_FILE="$LOG_DIR/$LOG"
ARCH_LOGFILE="$ARCH_LOG_DIR/${PARENTNAME_TMP}"_SFTP_"`date +"%Y%j%H%M%S"`.log"
...
while read TARGET_HOST_TXT SF_USER SOURCE_DIR_TXT SOURCE_FILE_TXT TARGET_DIR_TXT TARGET_FILE_TXT ACTION_TXT ARCHIVE_DIR_TXT ADDTL_CMD_TXT COMPRESS_IND 
do
...
if [[ COMPRESS_IND -eq 1 ]]; then
    ...
    print " "                                                                  >> $LOG_FILE
    print "No compression requested"                                           >> $LOG_FILE
    print " "                                                                  >> $LOG_FILE
fi
