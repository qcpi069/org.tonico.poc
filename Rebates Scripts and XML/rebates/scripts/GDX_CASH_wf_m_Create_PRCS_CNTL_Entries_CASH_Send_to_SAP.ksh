...
#-------------------------------------------------------------------------#
#
# Script        : GDX_Run_CASH_wf_m_Create_PRCS_CNTL_Entries_CASH_Send_to_SAP.ksh   
# Title         : 
#
# Description   : Searches Cash Recon for SAP Transactions that FAILED previously
#                 and are now ready to be re-transmitted.
#
# Maestro Job   : GD_3280 
#
# Parameters    : None
...
SCHEDULE="GDDY3000"
JOB="GD_3280"
FILE_BASE="GDX_Run_CASH_wf_m_Create_PRCS_CNTL_Entries_CASH_Send_to_SAP"
...
SQL_STMNT="Select count(*) from vrap.rcnt_recn_dstr_hdr hdr where hdr.dstr_stat_cd = 14 and hdr.dstr_crte_typ_cd in (1,4,5,11) "
...
if [[ $nr_row_count -gt 0 ]]; then
   SQL_STMNT="select count(*) from vrap.rcit_prcs_cntl pcnt where pcnt.prcs_cd = 23 and pcnt.prcs_stat_cd in (1,2,5,6,7) "
   print " "  >> $LOG_FILE
   print "We have $nr_row_count SAP Re-runs. Now look to see if there are processes running or waiting "  >> $LOG_FILE
   print "SQL_STMNT is - $SQL_STMNT"  >> $LOG_FILE
   db2 -x $SQL_STMNT > $ROW_CNT_FILE
   RETCODE=$?
   if [[ $RETCODE != 0 ]]; then
   ...
   if [[ $prcs_row_count -eq 0 ]]; then
    ...
    . $SCRIPTS_DIR/infa_pmcmd.ksh wf_m_Create_PRCS_CNTL_Entries_CASH_Send_to_SAP &
      RETCODE=$?
   else
      print " "  >> $LOG_FILE
      print "There are new re-run SAP Send entries that are in Ready To Tranmitt status "  >> $LOG_FILE 
