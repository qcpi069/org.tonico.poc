...
# 12-05-13    is00411    removed pkg creation/execution.
#
#-------------------------------------------------------------------------#
#-------------------------------------------------------------------------#
...
if [[ $REGION = "prod" ]];   then
    if [[ $QA_REGION = "true" ]];   then
        ...
        ALTER_EMAIL_ADDRESS='susan.pearson@caremark.com'
    else
...
#cat > $SQL_FILE << EOF
$ORACLE_HOME/bin/sqlplus -s $db_user_password <<EOF
set serveroutput on size 1000000
whenever sqlerror exit 1

dbms_output.put_line('dbms_snapshot.refresh(LIST=>''$OWNER.$SNAPSHOT'',METHOD=>''$Refresh_Type'',atomic_refresh=>false)')'
dbms_snapshot.refresh(LIST=>'$OWNER.$SNAPSHOT',METHOD=>'$Refresh_Type',atomic_refresh=>false);
...
#print " "                                                                      >> $LOG_FILE
#print "================================================================="      >> $LOG_FILE
#print " Now calling the Oracle package "                                       >> $LOG_FILE
#print `date`                                                                   >> $LOG_FILE
